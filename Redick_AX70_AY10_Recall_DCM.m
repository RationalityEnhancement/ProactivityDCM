...
dataRedick = dataRedick(dataRedick.Version == 70, :);
data_redick = table2array(dataRedick(dataRedick.Version == 70,[3,5,6,7,8]));
...
pA = pAX+pAY/(pAX+pAY+pBX+pBY); % + pAng + PBng
pB = pBX+pBY/(pAX+pAY+pBX+pBY); % + pAng + PBng
...
for s=1:nr_subjects
   [pars(s,:), fitted_accs(s,:), ll(s), fitted_p_intentions(s,:), fitted_optimal_cs(s,:), fitted_p_recalls(s,:)]= ...
...
mean_alpha = [mean(pars(:,1))] %, mean(alpha_cost_deltat_interference(reward==0,1)), mean(alpha_cost_deltat_interference(reward==1,1))]
mean_control_cost = [mean(pars(:,2))]%, mean(alpha_cost_deltat_interference(reward==0,2)), mean(alpha_cost_deltat_interference(reward==1,2))]
mean_delta_t = [mean(pars(:,3))]%, mean(alpha_cost_deltat_interference(reward==0,3)), mean(alpha_cost_deltat_interference(reward==1,3))]
mean_interference = [mean(pars(:,4))]%), mean(alpha_cost_deltat_interference(reward==0,4)), mean(alpha_cost_deltat_interference(reward==1,4))]
...
net_benefit_recall_X = @(ratio,l) E_delta_acc_recall_X(l)*ratio-mean_delta_t(1)-gamma%(ratio/4)-gamma;
net_benefit_recall_Y = @(ratio,l) E_delta_acc_recall_Y(l)*ratio-mean_delta_t(1)-gamma%(ratio/4)-gamma;
